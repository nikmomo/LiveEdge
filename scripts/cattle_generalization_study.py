...
CATTLE_BEHAVIOR_MAP = {
    ...
    # MODERATE cluster (feeding/standing) -> 12.5 Hz
    'standing': 'Standing',
    ...
    # ACTIVE cluster (locomotion) -> 25 Hz
    'walking': 'Walking',
...
CLUSTER_CONFIG = {
    'STABLE': {'behaviors': ['Lying'], 'odr': 6.25},
    'MODERATE': {'behaviors': ['Eating'], 'odr': 12.5},
    'ACTIVE': {'behaviors': ['Standing', 'Walking', 'Drinking', 'Interacting', 'Running'], 'odr': 25.0}
}
...
def extract_rfe20_features(X: np.ndarray) -> np.ndarray:
    """Extract RFE-20 features (same as pig study)."""
    n_windows = X.shape[0]
    features = np.zeros((n_windows, 20))

    for i in range(n_windows):
        acc_x, acc_y, acc_z = X[i, :, 0], X[i, :, 1], X[i, :, 2]
        ...
        band_0_2 = np.sum(fft_mag[(freqs >= 0) & (freqs < 2)]**2)
        band_2_5 = np.sum(fft_mag[(freqs >= 2) & (freqs < 5)]**2)
        band_5_10 = np.sum(fft_mag[(freqs >= 5) & (freqs < 10)]**2)
        band_10_25 = np.sum(fft_mag[(freqs >= 10) & (freqs < 25)]**2)
        total_power = np.sum(fft_mag**2) + 1e-10

        features[i, 0] = np.mean(mag)
        features[i, 1] = np.max(mag) - np.min(mag)
        features[i, 2] = np.std(mag)
        features[i, 3] = np.mean(acc_x)
        features[i, 4] = np.std(acc_x)
        features[i, 5] = np.mean(acc_y)
        features[i, 6] = np.std(acc_y)
        features[i, 7] = np.mean(acc_z)
        features[i, 8] = np.std(acc_z)
        features[i, 9] = np.percentile(mag, 75) - np.percentile(mag, 25)
        features[i, 10] = np.sum(np.diff(np.sign(mag - np.mean(mag))) != 0) / len(mag)
        features[i, 11] = band_0_2
        features[i, 12] = band_2_5
        features[i, 13] = (band_5_10 + band_10_25) / total_power
        features[i, 14] = np.mean(jerk_mag)
        features[i, 15] = np.std(jerk_x)
        features[i, 16] = np.std(jerk_y)
        features[i, 17] = np.std(jerk_z)
        features[i, 18] = np.sum(mag**2)
        features[i, 19] = np.sum(np.abs(acc_x) + np.abs(acc_y) + np.abs(acc_z))
...
def evaluate_5class_classification(X: np.ndarray, y: np.ndarray) -> Dict:
    ...
    # Extract features
    print(f"\n  Extracting RFE-20 features...")
    X_features = extract_rfe20_features(X_filtered)
...
def evaluate_energy_savings(X: np.ndarray, y: np.ndarray) -> Dict:
    ...
    # Energy calculation (using same model as pig study)
    # E_IMU = 0.155 * ODR (mJ/day simplified)
    baseline_energy = 0.155 * 50 * 86400 / 1000  # 50Hz baseline
    adaptive_energy = 0.155 * effective_odr * 86400 / 1000

    imu_savings = (baseline_energy - adaptive_energy) / baseline_energy * 100

    # Total energy (IMU + MCU, using pig study ratios)
    # Pig study: E_IMU = 13372, E_MCU = 844 (RFE-20), E_total = 7952 mJ/day
    # Scaling by ODR ratio
    baseline_total = 15331  # mJ/day at 50Hz (from pig study)

    # Scale by effective ODR
    odr_ratio = effective_odr / 50
    adaptive_imu = 13372 * odr_ratio
    adaptive_mcu = 844 * 0.2  # 20% inference rate
    adaptive_total = adaptive_imu + adaptive_mcu + 589  # + overhead

    total_savings = (baseline_total - adaptive_total) / baseline_total * 100

    # Battery life (CR2032: 220mAh, 80% usable, 3V)
    battery_capacity = 220 * 0.8 * 3600 * 3  # mAh * usable * s/h * V = mJ
    baseline_battery = battery_capacity / baseline_total
    adaptive_battery = battery_capacity / adaptive_total

    print(f"\n  Energy Analysis:")
    print(f"    Baseline (50Hz): {baseline_total:.0f} mJ/day")
    print(f"    Adaptive: {adaptive_total:.0f} mJ/day")
    print(f"    IMU Savings: {imu_savings:.1f}%")
    print(f"    Total Savings: {total_savings:.1f}%")
    print(f"    Battery Life: {baseline_battery:.0f} -> {adaptive_battery:.0f} days")
...
def compare_with_pig_study() -> Dict:
    """Compare cattle results with pig study results."""
    # Pig study results (from paper main results table)
    pig_results = {
        '5class_accuracy': 0.7924,
        '3cluster_accuracy': 0.8240,
        'cluster_f1': 0.7447,
        'total_savings_pct': 67.8,
        'effective_odr': 11.6,
        'battery_days': 385
    }
    return pig_results
